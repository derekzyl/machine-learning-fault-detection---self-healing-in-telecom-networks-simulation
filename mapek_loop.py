...
REMEDIATION_POLICY = {
    ...
    1: {"action": "Restore backup power + neighbour compensation (tilt/power boost)",
        "expected_recovery_s": 45},
    2: {"action": "Load balancing — HO offset adjust + PRB reallocation",
        "expected_recovery_s": 30},
    3: {"action": "Neighbour compensation — UL/DL power boost on adjacent cells",
        "expected_recovery_s": 50},
    -1:{"action": "Uncertain — conservative default: monitor + alert NOC",
...
def monitor_phase(current_window_kpis):
    ...
    triggered = (
        kpi_dict['prb_utilisation']   > REACTIVE_THRESHOLDS['prb_utilisation']  or
        kpi_dict['rsrp_avg_dbm']      < REACTIVE_THRESHOLDS['rsrp_avg_dbm']     or
        kpi_dict['packet_loss_rate']  > REACTIVE_THRESHOLDS['packet_loss_rate'] or
        kpi_dict['dl_throughput_mbps'] == 0.0
    )
    return triggered
...
def analyse_phase(window_arr, model, scaler_lstm, scaler_tab, pca, model_name):
    ...
    """
    X_seq, X_tab = extract_window_features(window_arr, scaler_lstm, scaler_tab, pca, model_name)
            ...
            if triggered:
                ...
                if pred_class not in [0, -1]:  # positive fault detection
                    ...
                    if in_fault and fault_onset is not None:
                        ...
                        recovery_events.append((sim_t + recovery_delay, fault_class_true, gnb_id, mttr))
...
def evaluate_reactive_baseline(df_test):
    ...
    for gnb_id, group in gnb_groups:
        ...
        in_fault = False; fault_onset = None; fault_class = None
        ...
        for t_idx in range(WINDOW_SIZE, len(group)):
            ...
            if not in_fault and true_label != 0:
                in_fault = True; fault_onset = sim_t; fault_class = true_label
            elif in_fault and true_label == 0:
                if fault_onset:
                    ...
                    mttrs.append(mttr)
                    downtime += mttr
                in_fault = False; fault_onset = None
...
def main():
    ...
    parser.add_argument("--model", default="all",
                        help="Model to evaluate: lstm | rf | svm | all")
    ...
    for m_name in model_names:
        try:
            ...
            mttr, avail, _ = evaluate_mapek(m_name, df_test, model,
                                             scaler_lstm, scaler_tab, pca)
            results[m_name.upper()] = (mttr, avail)
        except FileNotFoundError:
            print(f"  Warning: model file not found for {m_name}. "
    ...
    print(f"{'='*55}")
    print(f"  {'Condition':<30} {'MTTR (min)':>12} {'Availability':>14} {'MTTR Reduction':>16}")
    print(f"  {'-'*72}")
