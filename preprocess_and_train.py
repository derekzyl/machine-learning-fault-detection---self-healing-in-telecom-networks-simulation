...
def prepare_splits(X_seq, y):
    ...
    print(
        f"  PCA: {X_stat_train_n.shape[1]} → {X_tab_train_pca.shape[1]} components"
    ...
    sm = SMOTE(
        sampling_strategy=smote_strategy, random_state=RANDOM_STATE, k_neighbors=5
    )
    ...
    return {
        # LSTM
        "X_seq_train": X_seq_train_sm,
        "y_seq_train": y_seq_sm,
        "X_seq_val": X_seq_val_n,
        "y_val": y_val,
        "X_seq_test": X_seq_test_n,
        "y_test": y_test,
        # RF / SVM
        "X_tab_train": X_tab_train_sm,
        "y_tab_train": y_train_sm,
...
def train_svm(splits):
    ...
    print("  (using 20,000-sample subsample for speed)")
    print(f"{'=' * 55}")
    ...
    if len(y_tr) > 20000:
        sss2 = StratifiedShuffleSplit(
            n_splits=1, test_size=20000 / len(y_tr), random_state=RANDOM_STATE
        )
