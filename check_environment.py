...
def check_numpy_source(py=None):
    ...
    if "thesis-sim/venv" in loc:
        ...
        print(f"           It must come from ~/thesis-sim/venv/")
        print(f"           Fix: bash fix_python_env.sh")
        return False

# ════════════════════════════════════════════════════════════════════════════

print("\n" + "="*62)
print("  THESIS ENVIRONMENT CHECK  |  Peters PG/2415890")
print("="*62)
...
if in_venv and "thesis-sim/venv" in sys.executable:
    ...
    print(f"  [OK]     This script is running inside the venv")
    PY = sys.executable   # use current python
elif os.path.isfile(VENV_PY):
    print(f"  [WARN]   Venv exists but not currently activated")
    print(f"           Run: source ~/thesis-sim/activate_thesis.sh")
    print(f"           Checking venv packages directly anyway...")
    PY = VENV_PY
else:
    print(f"  [ERROR]  Venv not found at ~/thesis-sim/venv/")
    print(f"           Run: bash fix_python_env.sh")
    PY = sys.executable
...
check_cmd(f"test -f {ns3}/scratch/thesis-fault-sim.cc",  "thesis-fault-sim.cc in scratch/")
